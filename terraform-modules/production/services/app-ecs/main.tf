...
resource "aws_ecs_task_definition" "app_task" {
  ...
  container_definitions = jsonencode([
    {
      name = "${var.APP}-api-service",
      image = "${app_ecr.repository_url}:latest",
      environmentFiles = [
        {
          value = "${var.env_s3_arn}/appproduction.env",
          type = "s3"
        }
      ],
      essential = true,
      ...
      logConfiguration = {
        ...
        secretOptions = null,
        options = {
          awslogs-group = "/ecs/${var.APP}-log-group",
...
resource "aws_ecs_service" "app_service" {
  name            = "${var.APP}-service"     # Name the service
  ...
  load_balancer {
    ...
    container_name   = "${var.APP}-api-service"
    container_port   = 8000 # Specify the container port
  }
}

resource "aws_ecr_repository" "app_ecr" {
    name = "${var.APP}-repository"
