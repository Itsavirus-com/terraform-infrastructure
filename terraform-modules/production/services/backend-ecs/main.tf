resource "aws_ecs_task_definition" "app_task" {
  ...
  container_definitions = jsonencode([
    {
      name = "${var.BACKEND}-api-service",
      image = "${backend_ecr.repository_url}:latest",
      environmentFiles = [
        {
          value = "${var.env_s3_arn}/backendproduction.env",
          type = "s3"
        }
      ],
      essential = true,
      ...
      logConfiguration = {
        ...
        secretOptions = null,
        options = {
          awslogs-group = "/ecs/${var.BACKEND}-log-group",
...
resource "aws_ecs_service" "app_service" {
  name            = "${var.BACKEND}-service"     # Name the service
...
resource "aws_lb_target_group" "target_group" {
  name = "${var.BACKEND}-lb-target-group"
  port = 80
  protocol = "HTTP"
  vpc_id = var.vpc_id
}
...
resource "aws_cloudwatch_log_group" "ecs_service_log_group" {
  name = "/ecs/${var.BACKEND}-log-group"
  retention_in_days = 7
